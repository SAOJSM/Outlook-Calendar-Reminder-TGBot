...
SCOPE = ['https://graph.microsoft.com/Calendars.Read']  # 修改 scope 格式
...
def get_access_token():
    ...
    if token_data:
        ...
        # 檢查 token 是否過期
        if datetime.datetime.now().timestamp() - timestamp > expires_in - 300:  # 提前5分鐘更新
    ...
    if not access_token:
        logger.info("需要用戶授權，請選擇授權方式：")
        print("1. 在瀏覽器中開啟授權頁面（適用於本地環境）")
        print("2. 顯示授權 URL（適用於遠端環境）")
        ...
        if not auth_code or not acquire_token_by_auth_code(auth_code):
            return None
